...
function KrachtGrafiek() {
  ...
  const grafiekBreedte = Math.min(700, Math.max(280, window.innerWidth - 56));
  return (
    <AppScreen>
      ...
      {oefeningen.length === 0 ? <EmptyState icon="⌁" title="Nog geen krachtgegevens" description="Sla eerst een training met gewichten op." /> : <Card className="chart-card">
        ...
        {data.length === 0 ? <EmptyState title="Geen gegevens gevonden" /> : <div className="chart-wrap"><LineChart width={grafiekBreedte} height={250} data={data} margin={{ top: 8, right: 14, left: -18, bottom: 8 }}><CartesianGrid stroke={theme.colors.surfaceRaised} strokeDasharray="3 3" vertical={false} /><XAxis dataKey="training" stroke={theme.colors.textMuted} tick={{ fontSize: 11 }} /><YAxis stroke={theme.colors.textMuted} tick={{ fontSize: 11 }} /><Tooltip contentStyle={{ background: theme.colors.card, border: `1px solid ${theme.colors.border}`, borderRadius: 12 }} /><Line type="monotone" dataKey="gewicht" stroke={theme.colors.primary} strokeWidth={3} dot={{ fill: theme.colors.primary, r: 3 }} /></LineChart></div>}
      </Card>}
    </AppScreen>
  );
}
