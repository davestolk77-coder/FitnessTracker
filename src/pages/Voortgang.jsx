...
import { berekenGewichtGrafiekSchaal } from "../utils/weightChartScale";
...
function Voortgang() {
  ...
  const yAs = berekenGewichtGrafiekSchaal(metingen.map(({ gewicht }) => gewicht), "kg");
  return (
    <AppScreen>
      ...
      {metingen.length === 0 ? <EmptyState icon="↗" title="Nog geen metingen" description="Sla op het overzicht je gewicht op om hier je voortgang te zien." /> : <>
        <Card className="chart-card"><div className="chart-wrap"><ResponsiveContainer><LineChart data={metingen} margin={{ top: 8, right: 14, left: -18, bottom: 8 }}><CartesianGrid stroke={theme.colors.surfaceRaised} strokeDasharray="3 3" vertical={false} /><XAxis dataKey="datum" stroke={theme.colors.textMuted} tick={{ fontSize: 11 }} /><YAxis domain={yAs.domain} ticks={yAs.ticks} allowDecimals={false} stroke={theme.colors.textMuted} tick={{ fontSize: 11 }} /><Tooltip contentStyle={{ background: theme.colors.card, border: `1px solid ${theme.colors.border}`, borderRadius: 12 }} /><Line type="monotone" dataKey="gewicht" stroke={theme.colors.primary} strokeWidth={3} dot={{ fill: theme.colors.primary, r: 3 }} /></LineChart></ResponsiveContainer></div></Card>
